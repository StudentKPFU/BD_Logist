...
В первом эксперименте DELETE в сессии B будет ждать завершения транзакции A, потому что строка удерживается блокировкой FOR KEY SHARE, а удаление конфликтует с ней

Во втором эксперименте UPDATE в сессии B тоже будет ждать, так как FOR NO KEY UPDATE
